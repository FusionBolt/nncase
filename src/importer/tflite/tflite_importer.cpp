...
{
    auto ct = nncase::importer::add_node<ir::convert>(graph_, next_input, to_data_type(tensor.type()), get_shape(tensor.shape()), to_type);
    link_input_tensor(&ct->input(), tf_id);
}

void tflite_importer::convert_to_type(ir::input_connector &next_input, const tflite::Tensor &tensor, int32_t tf_id, datatype_t to_type)
{
